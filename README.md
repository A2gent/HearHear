...
6. Pause/resume with the same button. The left button restarts the current chunk; the right button cycles playback speed through 0.7x, 1x, 1.2x, 1.5x and 2x (remembered for later articles). Closing or navigating away from the source tab cancels synthesis and discards audio. A new article replaces the currently playing one.
